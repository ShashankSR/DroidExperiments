...
object Config {
    ...
    const val applicationTag = "Practo_Pro"
    const val applicationNameAlias = "PRACTO_PARTNER_ANDROID" // Don't change! Seriously! Don't!
    private const val STAGING_LATEST = "latest" // constant for latest staging
    
    @JvmStatic
    fun getBuildName(stagingName: String = "") =
        if ("latest" == stagingName) {
            "_latest"
        } else {
            "_$stagingName"
        }
    ...
    @JvmStatic
    fun getUrls(stagingName: String = ""): Map<String, String> {
        
        val domainSuffix =
            if (!stagingName.isEmpty() && stagingName != STAGING_LATEST) {
                "dev"
            } else {
                ""
            }
        
        val staging = if (!stagingName.isEmpty()) {
            "-$stagingName"
        } else {
            stagingName
        }
        ...
        val defaultUrl = "\"https://%s%s.practo%s.com%s\""
        
        return mapOf(
            "ACCOUNTS_URL" to String.format(defaultUrl, "accounts", staging, domainSuffix, ""),
            "RAY_URL" to String.format(defaultUrl, rayPrefix, staging, domainSuffix, raySuffix),
            "GDC_URL" to String.format(defaultUrl, rayPrefix, staging, domainSuffix, ""),
            "PRACTO_URL" to String.format(defaultUrl, "www", staging, domainSuffix, ""),
            "ONENESS_URL" to String.format(defaultUrl, "oneness", staging, domainSuffix, "")
        )
    }
}
...
object Versions {
    ...
    //network
    const val volleyplus_version = "0.1.3"
    ...
    const val mixpanel_version = "5.1.3"
    const val wootric_sdk_version = "2.5.1"
    const val onesignal_version = "3.10.1"
    const val appsflyer_version = "4.8.14"
    const val install_referrer_version = "1.0"
    
    //time
    const val joda_time_version = "2.9.1"
    
    // ui
    const val viewpager_indicator_version = "2.4.1"
    const val mpandroidchart_version = "2.2.4"
    const val floatingactingbutton_version = "1.10.0"
    const val unbescape_version = "1.1.1.RELEASE"
    const val header_recyclerview_version = "1.0.3"
    const val tools_layout_version = "1.10"
    const val sticky_header_rv_version = "0.4.3"
    const val material_datetime_picker_version = "2.3.0"
    const val otto_version = "1.3.6"
    
    const val fresco_version = "1.9.0"
    const val photodrawee_version = "1.1.3"
    const val mockk_version = "1.8.4"
    const val clevertap_version = "3.1.10"
...
object Libraries {
    ...
    const val mixpanel = "com.mixpanel.android:mixpanel-android:${Versions.mixpanel_version}"
    const val wootric = "com.wootric:wootric-sdk-android:${Versions.wootric_sdk_version}"
    const val materialDateTimePicker =
        "com.wdullaer:materialdatetimepicker:${Versions.material_datetime_picker_version}"
    const val mpAndroidChart = "com.github.PhilJay:MPAndroidChart:v${Versions.mpandroidchart_version}"
    const val volley = "dev.dworks.libs:volleyplus:${Versions.volleyplus_version}"
    const val appsflyer = "com.appsflyer:af-android-sdk:${Versions.appsflyer_version}@aar"
    const val floatingActionButton = "net.i2p.android.ext:floatingactionbutton:${Versions.floatingactingbutton_version}"
    const val unbescape = "org.unbescape:unbescape:${Versions.unbescape_version}"
    const val headerRecyclerView = "com.karumi:headerrecyclerview:${Versions.header_recyclerview_version}"
    const val toolsLayout = "org.apmem.tools:layouts:${Versions.tools_layout_version}@aar"
    const val stickyHeadersRecyclerView =
        "com.timehop.stickyheadersrecyclerview:library:${Versions.sticky_header_rv_version}@aar"
    ...
    const val otto = "com.squareup:otto:${Versions.otto_version}"
    const val viewPagerIndicator = "com.github.JakeWharton:ViewPagerIndicator:${Versions.viewpager_indicator_version}"
    ...
    const val clevertap = "com.clevertap.android:clevertap-android-sdk:${Versions.clevertap_version}"
    const val jodaTime = "joda-time:joda-time:${Versions.joda_time_version}"
